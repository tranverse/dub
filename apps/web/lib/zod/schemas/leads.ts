...
export const trackLeadRequestSchema = z.object({
  clickId: z
    ...
    .trim()
    .describe(
      "The unique ID of the click that the lead conversion event is attributed to. You can read this value from `dub_id` cookie. [For deferred lead tracking]: If an empty string is provided, Buzz will try to find an existing customer with the provided `customerExternalId` and use the `clickId` from the customer if found.",
    ),
  eventName: z
    .string()
    .trim()
